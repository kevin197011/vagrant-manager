...
def generate_new_tag
  # 获取所有 tag
  tags_output = `git tag -l 'v*' 2>&1`
  return 'v2.8.4' unless $?.success? && !tags_output.empty?

  # 解析版本号并排序
  tags = tags_output.split("\n").select { |t| t.match?(/^v\d+\.\d+\.\d+$/) }
  return 'v2.8.4' if tags.empty?

  # 获取最新的 tag
  latest_tag = tags.sort_by do |tag|
    version = tag.gsub(/^v/, '').split('.').map(&:to_i)
    [version[0], version[1], version[2]]
  end.last

  # 解析版本号
  version_parts = latest_tag.gsub(/^v/, '').split('.').map(&:to_i)
...
def create_and_push_tag(tag_name, commit_message)
  # 检查 tag 是否已存在
  tag_check = `git tag -l #{tag_name} 2>&1`
  ...
  tag_message = commit_message.lines.first.chomp
  success = system("git tag -a #{tag_name} -m \"#{tag_message}\" 2>&1")
  ...
  puts "✅ 创建 tag: #{tag_name}"

  # 推送 tag 到远程
  push_tag_output = `git push origin #{tag_name} 2>&1`
  unless $?.success?
    puts "❌ 推送 tag 失败"
    puts push_tag_output
    return false
  end

  puts "✅ 推送 tag 成功: #{tag_name}"
  true
end
...
task :push do
  ...
  create_and_push_tag(new_tag, commit_message)
end
